...
export default function Home(props) {
  ...
  const moveMarker = () =>{
    ...
    var selfPos = document.getElementsByClassName("button_amazonia")[0].getClientRects()[0]
    //console.log("[marcador]", pos)

    var tMoveMarker = gsap.timeline({paused: true})
    .add(gsap.set(".button_amazonia", {scale:0, duration: 1}), 0)
    .add(gsap.set(".button_amazonia", {top: targetPos.top + targetPos.height/2 - selfPos.height, duration: 1}), 0)
    .add(gsap.set(".button_amazonia", {left: targetPos.left + targetPos.width/2 - selfPos.width/2, duration: 1}),0)
    ...
    galeryData.forEach((photo)=>{
      ...
      tMoveMarker.add(gsap.set("."+photo.className, {scale:0, duration: 1}), 0)
      tMoveMarker.add(gsap.set("."+photo.className, {top: photo.top(targetPos, selfPos), duration: 1}), 0)
